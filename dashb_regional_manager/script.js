document.addEventListener('DOMContentLoaded', function() {
    ...
    const menuButton = document.querySelector('.menu-btn'); // Select the menu button
    const branchSelector = document.querySelector('.branch-selector'); // Select the branch selector
    const sidebar = document.querySelector('.sidebar'); // Select the sidebar

    menuButton.addEventListener('click', function() {
        branchSelector.classList.toggle('show'); // Toggle visibility of branch selector
        dashboardContainer.classList.toggle('nav-open'); // Also toggle the nav-open class
    ...
    document.addEventListener('click', function(e) {
        if (dashboardContainer.classList.contains('nav-open') &&
            ...
            branchSelector.classList.remove('show'); // Hide branch selector
        }
    });
    ...
    // Add click event listener to each branch in the sidebar
    const sidebarBranchItems = document.querySelectorAll('.sidebar .branch-item');

    sidebarBranchItems.forEach(branch => {
        branch.addEventListener('click', function() {
            // Logic to navigate to the selected branch
            // For example, you can update the main content area based on the selected branch
            const selectedBranch = this.innerText;
            console.log(`Navigating to ${selectedBranch}`); // Placeholder for actual navigation logic
        });
    });
}); 